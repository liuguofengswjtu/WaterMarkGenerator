...
def build():
    ...
    # 图标文件（平台特定）
    if is_mac:
        icon_file = os.path.join(resources_dir, "app_icon.icns")
        if not os.path.exists(icon_file):
            print(f"警告: macOS 图标 {icon_file} 不存在，将尝试使用 app_icon.ico 或跳过图标")
            icon_file = os.path.join(resources_dir, "app_icon.ico") if os.path.exists(os.path.join(resources_dir, "app_icon.ico")) else None
    else:
        icon_file = os.path.join(resources_dir, "app_icon.ico")
    
    # 构建 --add-data 参数（分隔符平台特定）
    sep = ':' if is_mac else ';'
    add_data_args = []
    for f in resource_files:
        src = os.path.join(resources_dir, f)
        if os.path.exists(src):
            add_data_args.append(f"--add-data={src}{sep}.")
        else:
            print(f"警告: 资源文件 {src} 不存在，将跳过")
    ...
    cmd = [
        ...
        "--clean",          # 清理临时文件
        "--noconfirm",      # 不询问确认
        "--paths=src",      # 添加 src 目录到 Python 路径，确保能找到 watermark_core
        "--hidden-import=PyQt5.QtSvg",  # PyInstaller 无法自动发现 try-except 中的导入
    ]
