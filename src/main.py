...
# 确保脚本所在目录在导入路径中（兼容 PyInstaller 打包环境）
...
def _get_resource_path(filename):
    ...
    # 开发环境：src/main.py -> 项目根目录 -> resources/
...
class BatchWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if failed_files:
                fail_msg = "\n".join(failed_files)
                self.finished_signal.emit(False, f"成功 {success_count} 张，失败 {len(failed_files)} 张\n输出目录: {self.output_dir}\n\n失败详情:\n{fail_msg}")
            else:
                self.finished_signal.emit(True, f"成功处理 {success_count} 张图片\n输出目录: {self.output_dir}")
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.last_output_dir = None  # 上次使用的输出目录，避免重复选择
        self.preview_params = {
            'font_path': None,
